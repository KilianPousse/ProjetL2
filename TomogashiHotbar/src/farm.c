...
void displayPlant( plant_t p ){
    ...
    SDL_Texture * texture = SDL_CreateTextureFromSurface(renderer, farm.texture[p.id][p.age]);
    if (texture == NULL) {
...
int initFarmTexture(  ){
    ...
    for( int i=0; i<NB_STEM_PLANT; i++ ){
        ...
        farm.texture[ID_PLANT_BLE][i] = IMG_Load( file_name );
    ...
    for( int i=0; i<NB_STEM_PLANT; i++ ){
        ...
        farm.texture[ID_PLANT_TOMATE][i] = IMG_Load( file_name );
